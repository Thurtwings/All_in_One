...
namespace ThurtwingsGames
{
    public class ManagerStack : MonoBehaviour
    {
        ...
        [Range(1f, 2f)]
        public float tileMaxOverMove = 1.4f;
        ...
        internal float tileSpeed = 2f;

        private float tileTransition = 0.0f;
        private float secondaryPosition;

        private bool isOnXAxis = true;
        private bool isGameOver = false;

        private Vector3 desiredPosition;
        private Vector3 lastTilePosition;

        private Vector2 stackBounds = new(BOUNDS_SIZE, BOUNDS_SIZE);
        ...
        private void MoveTile()
        {
            ...
            if (isOnXAxis)
                theStack[stackIndex].transform.localPosition = new Vector3(Mathf.Sin(tileTransition) * BOUNDS_SIZE * tileMaxOverMove, scoreCount, secondaryPosition);
            else
                theStack[stackIndex].transform.localPosition = new Vector3(secondaryPosition, scoreCount, Mathf.Sin(tileTransition) * BOUNDS_SIZE * tileMaxOverMove);
        }
        ...
        private bool PlaceTile()
        {
            ...
            if (isOnXAxis)
            {
                float deltaPositionX = lastTilePosition.x - t.position.x;
                if (Mathf.Abs(deltaPositionX) > ERROR_MARGIN)
                {
                    //Coupe la tile
                    combo = 0;
                    
                    stackBounds.x -= Mathf.Abs(deltaPositionX);
                    ...
                    (
                        new Vector3((t.position.x > 0)
                            ...
                            , t.position.y
                            , t.position.z),
                        new Vector3(Mathf.Abs(deltaPositionX), 1, t.localScale.y)
                    );
                    t.localPosition = new Vector3(middle - (lastTilePosition.x / 2), scoreCount, lastTilePosition.z);
            ...
            {
                float deltaPositionZ = lastTilePosition.z - t.position.z;
                if (Mathf.Abs(deltaPositionZ) > ERROR_MARGIN)
                {
                    //Coupe la tile
                    combo = 0;

                    stackBounds.y -= Mathf.Abs(deltaPositionZ);
                    if (stackBounds.y <= 0f)
                    {
                        return false;
                    }
                    ...
                    (
                        new Vector3(t.position.x
                            ...
                            ? t.position.z + (t.localScale.z / 2)
                            : t.position.z - (t.localScale.z / 2)),
                        new Vector3(Mathf.Abs(deltaPositionZ), 1, t.localScale.z)
                    );
                    t.localPosition = new Vector3(lastTilePosition.x, scoreCount, middle - (lastTilePosition.z / 2));
